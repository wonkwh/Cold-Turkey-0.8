...
Public Class ColdTurkey
    ...
    Dim hostDirS As String = Environ("WinDir") & "\system32\drivers\etc\hosts"
    Dim svcAlreadyExistsB As Boolean = False
    ...
    Private Sub Form1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Catch
            MsgBox("Cold Turkey encountered an error connecting to the Internet.")
            End
        End Try
    ...
    Private Sub addWindowsDefenderException()

        Dim bit As String = WindowsBitVersion()

        Dim setACLSetOwner As New ProcessStartInfo
        setACLSetOwner.FileName = Application.StartupPath & "\SetACL " & bit & "\SetACL.exe"
        setACLSetOwner.Arguments = "-on ""HKLM\SOFTWARE\Microsoft\Windows Defender\Exclusions\Paths"" -ot reg -actn setowner -ownr ""n:Administrators"""
        setACLSetOwner.WindowStyle = ProcessWindowStyle.Hidden
        Process.Start(setACLSetOwner)

        Dim setACL As New ProcessStartInfo
        setACL.FileName = Application.StartupPath & "\SetACL " & bit & "\SetACL.exe"
        setACL.Arguments = "-on ""HKLM\SOFTWARE\Microsoft\Windows Defender\Exclusions\Paths"" -ot reg -actn ace -ace ""n:Administrators;p:full"""
        setACL.WindowStyle = ProcessWindowStyle.Hidden
        Process.Start(setACL)

        Try
            If Registry.LocalMachine.OpenSubKey("SOFTWARE\Microsoft\Windows Defender\Exclusions\Paths").Equals(Nothing) Then
            Else
                'REG ADD "HKLM\SOFTWARE\Microsoft\Windows Defender\Exclusions\Paths" /v C:\Windows\System32\Drivers\etc\hosts /d 0x00000000 /t REG_DWORD /f
                Dim regKey As RegistryKey = Registry.LocalMachine.OpenSubKey("SOFTWARE\Microsoft\Windows Defender\Exclusions\Paths", True).GetValue(hostDirS)
                If regKey Is Nothing Then
                    regKey = Registry.LocalMachine.CreateSubKey("SOFTWARE\Microsoft\Windows Defender\Exclusions\Paths")
                    regKey.SetValue(hostDirS, 0)
                Else
                End If
            End If
        Catch
    ...
    Private Function WindowsBitVersion() As String
    ...
    Private Function writeToHostsFileTest() As Boolean
        ...
        If My.Computer.FileSystem.FileExists(hostDirS) Then
            Try
                ...
                sw.WriteLine(vbNewLine + "#### Cold Turkey Write Test ####")
                sw.Close()

                Threading.Thread.Sleep(500)

                fileReader = My.Computer.FileSystem.ReadAllText(hostDirS)
                If (Not fileReader.Contains("#### Cold Turkey Write Test ####")) Then
                    Return False
                End If

                startpos = InStr(1, fileReader, "#### Cold Turkey Write Test ####")
    ...
    Private Sub writeToHostsFile()
        ...
        Try
            ...
            If chk_shotmail.Checked = True Then
                ...
                + vbNewLine + "0.0.0.0 outlook.com" _
                + vbNewLine + "0.0.0.0 www.outlook.com" _
    ...
    Private Sub startService()
        ...
        Try
            ...
            Dim dateToBlockToString As String = DateTimePicker1.Value.Date & " " & hour24I & ":" & minute.Text & ":00"
            iniFile.Load(Application.StartupPath + "\ct_settings.ini")
            iniFile.SetKeyValue("Time", "Until", encryptionW.EncryptData(Convert.ToInt64((ntcTime - epoch).TotalSeconds) + DateDiff(DateInterval.Second, DateTime.Now, DateTime.Parse(dateToBlockToString))))
        ...
        Catch ex As Exception
            ...
            erroredOut()
        End Try
        If svcAlreadyExistsB = False Then
    ...
    Private Sub resetConfigFile()
        ...
        Try
            ...
            Application.Restart()
        Catch ex As Exception
            MsgBox("An error occured trying to fix your configuration file.")
            End
        End Try
    End Sub
    ...
    Private Sub erroredOut()
        ...
        Try
            ...
            iniFile.Save(Application.StartupPath + "\ct_settings.ini")
        Catch ex As Exception
            MsgBox("Cold Turkey encountered an error writing to its configuration file.")
        End Try
    ...
    Private Sub lbl_info_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles lbl_info.Click
        ...
        Dim bit As String = WindowsBitVersion()
        message = "Cold Turkey 0.8 running in " & bit & "bit mode." & vbNewLine & "(c) Copyright Felix Belzile, 2013" & vbNewLine & vbNewLine & "Cold Turkey will temporarily block addicting sites and games" & vbNewLine & "so that you can get some work done!"
    ...
    Private Sub add_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles add.Click
        ...
        Dim tempString As String = txt_add.Text
